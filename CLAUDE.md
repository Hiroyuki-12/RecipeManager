# RecipeManager 運用ルール (Claude Code 用)
...
例:
- `feat/#12-add-recipe-form`
- `fix/#34-recipe-delete-500`
...
- PR 上のレビューコメント / 会話は **全解決してからマージ**
- マージ方式は **squash または rebase のみ** (merge commit 禁止 / 線形履歴必須)
- マージ後はブランチを自動削除する設定

## main ブランチ保護 (GitHub 側で強制する想定)
...
このプロジェクトで使用するポートは以下に固定する。**別ポートでの代替起動は禁止**（Next.js から Rails API を叩く前提や README の手順が崩れて動作確認にならないため）。

Next.js と Rails はどちらもデフォルトポートが `3000` で衝突するため、**Rails 側を `3001` にずらす**運用とする。
...
2. **そのプロセスを停止する**（`kill <PID>` / `docker stop <container>` 等、適切な手段で）
3. その上で本来のポートで起動し直す

絶対に行ってはいけないこと:

- `next dev -p 3002` 等で別ポートに逃げる
- `bin/rails s -p 3002` のように別ポートに逃げる
- `config/puma.rb` や環境変数の `PORT` を一時的に変更する
...
**PR 作成前に必ず `/quality-review` skill を実行すること。** 指摘事項があれば修正してから PR を出す。

- 各フェーズ完了時（コード変更を含む全 PR）に適用する
- 最終フェーズ完了後にも全体の品質チェックを行う
- 品質チェックをスキップして PR を作成してはならない
...
2. 上記命名規則に従ってブランチを切る
3. 変更をコミットして push
4. **`/quality-review` を実行し、指摘があれば修正する**
5. `gh pr create` で PR を作成し、本文に `Closes #<issue>` を入れる
6. `main` への直接コミット / push は絶対に行わない
